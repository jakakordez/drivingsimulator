...
namespace Driving_simulator
{
    ...
    class Lane
    {
        ...
        public Vector3[] LeftBorder, RightBorder;
        public Vector3[] Points;
        public Lane() { LeftBorder = new Vector3[0]; RightBorder = new Vector3[0]; }
        ...
        public void GenerateRoad(Vector2[] roadLine, int Segments, float Offset, float Width, float Height)
        {
            ...
            Points = new Vector3[roadCurve.Length * 4];
            float kot = 0;
            Width /= 2;
            ...
            for (int i = 0; i < roadCurve.Length; i++)
            {
                Points[i] = new Vector3(pointsL[i].X, Height, pointsL[i].Y);
                Points[i] = new Vector3(pointsR[i].X, Height, pointsR[i].Y);
            }
            for (int i = roadCurve.Length-1; i >= 0 ; i--)
            {
                Points[roadCurve.Length+i] = new Vector3(pointsL[i].X, 0, pointsL[i].Y);
                Points[roadCurve.Length+i] = new Vector3(pointsL[i].X, Height, pointsL[i].Y);
            }
            for (int i = 0; i < roadCurve.Length; i++)
            {
                Points[(roadCurve.Length*2)+i] = new Vector3(pointsR[i].X, 0, pointsR[i].Y);
                Points[(roadCurve.Length * 2) + i] = new Vector3(pointsR[i].X, Height, pointsR[i].Y);
            }
        }
    }
    class ObjectLine
